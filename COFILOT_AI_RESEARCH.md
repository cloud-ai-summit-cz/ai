...
terraform init
cp demo.tfvars my.tfvars   # Edit with your values
terraform apply -var-file=my.tfvars
```

This creates: Resource Group, Container Apps Environment, ACR, API Management, Azure AI Foundry, and Log Analytics.

### 2. Build & Push All Containers

```bash
cd deploy/azure
uv sync
uv run python build.py   # Builds all 13 containers to ACR
```

Uses ACR Tasks (no local Docker required). See `build-config.yaml` for the container list.

### 3. Access the Application

- **Web UI**: `https://web-research.<your-domain>.azurecontainerapps.io`
- **API**: Via Azure API Management gateway
