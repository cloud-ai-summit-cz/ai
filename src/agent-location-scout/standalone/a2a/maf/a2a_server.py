"""A2A Server implementation for the Location Scout Agent.

This module implements the A2A (Agent-to-Agent) protocol server that wraps
the Microsoft Agent Framework Location Scout agent, making it accessible
...
class LocationScoutExecutor(AgentExecutor):
    """A2A AgentExecutor implementation for the Location Scout.
    ...
    def __init__(self) -> None:
        ...
        self._shared_agent: LocationScoutAgent | None = None

    async def _get_agent(self, session_id: str | None = None) -> LocationScoutAgent:
        ...
        if session_id:
            ...
            agent = LocationScoutAgent(self._settings, session_id=session_id)
            await agent.initialize()
            return agent
        else:
            # Use shared agent for non-session requests
            if self._shared_agent is None:
                self._shared_agent = LocationScoutAgent(self._settings)
    ...
    async def execute(
        ...
        Location Scout agent, and produces the appropriate A2A response.
        ...
        """
        session_scoped_agent: LocationScoutAgent | None = None
        try:
            ...
            await updater.add_artifact(
                ...
                name="Location Analysis Response",
            )
...
def create_agent_card(port: int) -> AgentCard:
    """Create the A2A AgentCard for the Location Scout.
    ...
    return AgentCard(
        ...
        skills=[
            AgentSkill(
                ...
                description=(
                    "Analyze neighborhoods and districts for Cofilot's brand fit, "
                    "including vibe assessment, key landmarks, and suitability scoring."
                ),
                tags=["neighborhood", "district", "analysis", "brand-fit"],
                examples=[
                    "Analyze the Veveří district in Brno for coffee shop expansion",
                    "What's the vibe of Vienna's 7th district (Neubau)?",
                    "Compare Brno city center vs Královo Pole for our brand",
                ],
            ),
            AgentSkill(
                id="demographics-analysis",
                name="Demographics Analysis",
                description=(
                    "Analyze population demographics including age distribution, "
                    "income levels, education, and occupation mix for target areas."
                ),
                tags=["demographics", "population", "income", "education"],
                examples=[
                    "What's the demographic profile of Brno Veveří?",
                    "Income levels in Vienna Neubau vs Leopoldstadt",
                    "Student population density near Masaryk University",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Estimate pedestrian traffic patterns including daily counts, "
                    "peak hours, and nearby foot traffic generators."
                ),
                tags=["foot-traffic", "pedestrian", "patterns", "peak-hours"],
                examples=[
                    "What's the foot traffic like in Brno city center?",
                    "Peak hours for Vienna Mariahilfer Straße",
                    "Weekend vs weekday traffic patterns in target areas",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Assess commercial real estate availability, rental rates, "
                    "lease terms, and property suitability for coffee shop operations."
                ),
                tags=["real-estate", "rental", "commercial", "property"],
                examples=[
                    "What are rental rates for retail space in Brno?",
                    "Available café spaces in Vienna 7th district",
                    "Typical lease terms for commercial space in target areas",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Research business licenses, permits, zoning requirements, "
                    "and regulatory compliance for Czech Republic and Austria."
                ),
                tags=["permits", "licenses", "regulations", "zoning", "compliance"],
                examples=[
                    "What permits do we need to open a café in Brno?",
                    "Austria Gastgewerbekonzession requirements",
                    "Zoning restrictions for food service in Vienna",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Provide comprehensive location recommendations with "
                    "suitability scores, pros/cons, and time-to-open estimates."
                ),
                tags=["recommendation", "suitability", "scoring", "comparison"],
                examples=[
                    "Recommend the best district for Cofilot in Brno",
                    "Compare Vienna districts for our expansion",
                    "Which location should we prioritize and why?",
                ],
            ),
        ],
    )
...
def create_app(port: int) -> A2AStarletteApplication:
    ...
    executor = LocationScoutExecutor()
