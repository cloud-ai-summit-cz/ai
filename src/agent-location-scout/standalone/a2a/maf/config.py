"""Configuration for standalone Location Scout A2A Agent."""
...
class Settings(BaseSettings):
    ...
    # MCP Government Data Configuration (permits, regulations, zoning)
    mcp_government_data_url: str
    mcp_government_data_api_key: str

    # MCP Demographics Configuration (population, income, foot traffic)
    mcp_demographics_url: str
    mcp_demographics_api_key: str

    # MCP Real Estate Configuration (property listings, rental rates)
    mcp_real_estate_url: str
    mcp_real_estate_api_key: str

    # A2A Server Configuration
    a2a_server_host: str = "0.0.0.0"
    a2a_server_port: int = 8023
    ...
    a2a_agent_name: str = "Location Scout Agent"
    a2a_agent_description: str = (
        "Location scouting specialist for Cofilot's coffee business expansion. "
        "Analyzes neighborhoods, districts, real estate availability, regulatory requirements, "
        "demographics, foot traffic patterns, and site selection criteria for Brno and Vienna."
    )
    ...
    def prompts_dir(self) -> Path:
        ...
        # Default: navigate from standalone/a2a/maf/ up to agent-location-scout/prompts/
        current_dir = Path(__file__).parent
        return current_dir.parent.parent.parent / "prompts"
