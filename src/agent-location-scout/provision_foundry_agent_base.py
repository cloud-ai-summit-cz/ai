"""Provision Location Scout as a prompt-only agent in Azure AI Foundry.
...
For the HOSTED agent deployment (LangGraph container), see:
  standalone/foundry/langgraph/provision.py

MCP tools injected by orchestrator:
- mcp-scratchpad: Session-scoped shared memory
- mcp-real-estate: Rental rates, property data
- mcp-government-data: Permits, regulations
- mcp-demographics: Population, income data
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        ...
        console.print("\n[bold]For hosted agent deployment:[/bold]")
        console.print("  cd standalone/foundry/langgraph")
        console.print("  uv run python provision.py deploy")
        sys.exit(1)
