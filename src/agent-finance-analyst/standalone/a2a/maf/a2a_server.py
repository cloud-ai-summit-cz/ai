"""A2A Server implementation for the Finance Analyst Agent.

This module implements the A2A (Agent-to-Agent) protocol server that wraps
the Microsoft Agent Framework Finance Analyst agent, making it accessible
...
from agent import FinanceAnalystAgent
...
class FinanceAnalystExecutor(AgentExecutor):
    """A2A AgentExecutor implementation for the Finance Analyst.
    ...
    async def execute(
        ...
        Finance Analyst agent, and produces the appropriate A2A response.
        ...
        try:
            ...
            await updater.add_artifact(
                ...
                name="Finance Analysis Response",
            )
...
def create_agent_card(port: int) -> AgentCard:
    """Create the A2A AgentCard for the Finance Analyst.
    ...
    return AgentCard(
        ...
        skills=[
            AgentSkill(
                ...
                description=(
                    "Comprehensive financial analysis including startup costs, "
                    "operating expenses, revenue projections, and profitability assessment."
                ),
                tags=["financial", "viability", "startup", "operating", "costs"],
                examples=[
                    "What are the estimated startup costs for a coffee shop in Vienna?",
                    "Calculate monthly operating expenses for a Brno location",
                    "Is this location financially viable?",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Calculate break-even points, payback periods, "
                    "and time to profitability for potential locations."
                ),
                tags=["break-even", "payback", "profitability", "timeline"],
                examples=[
                    "When will the Vienna location break even?",
                    "What's the payback period for the Brno investment?",
                    "How long until this location becomes profitable?",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Calculate key investment metrics including ROI, NPV, IRR, "
                    "and cash flow projections."
                ),
                tags=["roi", "npv", "irr", "investment", "returns", "cash-flow"],
                examples=[
                    "What's the expected ROI for the Vienna expansion?",
                    "Calculate NPV for the Brno location over 5 years",
                    "Project cash flow for the first 12 months",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Analyze how changes in key variables affect financial outcomes "
                    "and identify critical risk factors."
                ),
                tags=["sensitivity", "risk", "scenarios", "variables"],
                examples=[
                    "How does rent increase affect profitability?",
                    "What if revenue is 20% lower than projected?",
                    "Run sensitivity analysis on key cost variables",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Project revenue based on market size, foot traffic, "
                    "pricing strategy, and competitive positioning."
                ),
                tags=["revenue", "projection", "forecast", "sales"],
                examples=[
                    "Project monthly revenue for the Vienna location",
                    "What revenue can we expect in year one?",
                    "Estimate sales based on foot traffic data",
                ],
            ),
        ],
    )
...
def create_app(port: int) -> A2AStarletteApplication:
    ...
    executor = FinanceAnalystExecutor()
