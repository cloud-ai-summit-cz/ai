"""A2A Server implementation for the Competitor Analyst Agent.

This module implements the A2A (Agent-to-Agent) protocol server that wraps
the Microsoft Agent Framework Competitor Analyst agent, making it accessible
...
class CompetitorAnalystExecutor(AgentExecutor):
    """A2A AgentExecutor implementation for the Competitor Analyst.
    ...
    def __init__(self) -> None:
        ...
        self._shared_agent: CompetitorAnalystAgent | None = None

    async def _get_agent(self, session_id: str | None = None) -> CompetitorAnalystAgent:
        ...
        if session_id:
            ...
            agent = CompetitorAnalystAgent(self._settings, session_id=session_id)
            await agent.initialize()
            return agent
        else:
            # Use shared agent for non-session requests
            if self._shared_agent is None:
                self._shared_agent = CompetitorAnalystAgent(self._settings)
    ...
    async def execute(
        ...
        Competitor Analyst agent, and produces the appropriate A2A response.
        ...
        """
        session_scoped_agent: CompetitorAnalystAgent | None = None
        try:
            ...
            await updater.add_artifact(
                ...
                name="Competitor Analysis Response",
            )
...
def create_agent_card(port: int) -> AgentCard:
    """Create the A2A AgentCard for the Competitor Analyst.
    ...
    return AgentCard(
        ...
        skills=[
            AgentSkill(
                ...
                description=(
                    "Map all relevant players in the specialty coffee market - "
                    "chains, independents, and specialty cafés in target cities."
                ),
                tags=["competitor", "mapping", "landscape", "identification"],
                examples=[
                    "Who are the main competitors in the Vienna specialty coffee market?",
                    "List coffee shops in Brno that could compete with Cofilot",
                    "Identify third-wave coffee players in the target market",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Create detailed profiles of top competitors including positioning, "
                    "pricing, strengths, weaknesses, and threat assessment."
                ),
                tags=["profile", "analysis", "strengths", "weaknesses", "pricing"],
                examples=[
                    "Profile the top 5 specialty coffee competitors in Vienna",
                    "What are Starbucks' strengths and weaknesses in this market?",
                    "Analyze Balthasar Vienna's competitive positioning",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Analyze competitive positioning, identify white spaces, "
                    "and recommend differentiation strategies."
                ),
                tags=["positioning", "differentiation", "white-space", "strategy"],
                examples=[
                    "Where are the positioning gaps in the Vienna coffee market?",
                    "How should Cofilot differentiate from local competitors?",
                    "Map competitors on quality vs. price positioning",
                ],
            ),
            AgentSkill(
                ...
                description=(
                    "Assess competitive intensity, barriers to entry, "
                    "and potential threats to market entry."
                ),
                tags=["threats", "barriers", "entry", "intensity", "risk"],
                examples=[
                    "What are the main barriers to entry in Vienna's coffee market?",
                    "Assess the competitive threat level from existing players",
                    "What risks do local chains pose to Cofilot's expansion?",
                ],
            ),
        ],
    )
...
def create_app(port: int) -> A2AStarletteApplication:
    ...
    executor = CompetitorAnalystExecutor()
