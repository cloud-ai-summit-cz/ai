"""Agent provisioner for Invoice Validation Summary.

Creates and destroys the invoice-validation-summary-agent in Azure AI Foundry Agent Service.
This agent summarizes validation results for downstream consumers.
"""
...
AGENT_NAME = "invoice-validation-summary-agent"
AGENT_DISPLAY_NAME = "Invoice Validation Summary Agent"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="provision.py",
        description="Provision and manage the Invoice Validation Summary agent in Azure AI Foundry.",
    )
...
def create_agent() -> None:
    """Create the invoice-validation-summary-agent in AI Foundry.
        ...
        if existing:
            ...
            with console.status(f"Deleting existing {AGENT_NAME}..."):
                try:
                    ...
                    console.print(
                        f"[yellow]Warning: Could not delete {AGENT_NAME}: {e}[/yellow]"
                    )
        ...
        with console.status(f"Creating {AGENT_DISPLAY_NAME}..."):
            try:
                ...
                client.agents.create(
                    ...
                    metadata={
                        "workflow_sequence": "2.5",
                        "workflow_role": "validation-summary",
                ...
                console.print("  Workflow Sequence: 2.5")
                console.print("  Workflow Role: validation-summary")
                console.print("  Output: Text summary")
...
def destroy_agent() -> None:
    """Destroy the invoice-validation-summary-agent."""
