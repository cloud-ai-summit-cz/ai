# Container App for Location Scout A2A Agent
# This agent uses Microsoft Agent Framework with A2A protocol
# Uses MCP Government Data, MCP Demographics, MCP Real Estate, MCP Scratchpad, and Grounded Web Search (Bing)

resource "azapi_resource" "capp_agent_location_scout_a2a" {
  type      = "Microsoft.App/containerApps@2024-03-01"
  name      = "ca-agent-location-scout-a2a"
        ...
        containers = [
          {
            ...
            image = local.agent_location_scout_a2a_image
            resources = {
            ...
            env = [
              ...
              },
              {
                ...
                value = "ca-agent-location-scout-a2a.${azapi_resource.container_app_environment.output.properties.defaultDomain}"
              },
              {
                name      = "A2A_API_KEY"
              ...
              # MCP Government Data Configuration (via APIM) - permits, regulations, zoning
              {
              ...
              # MCP Demographics Configuration (via APIM) - population, income, foot traffic
              {
              ...
              # MCP Real Estate Configuration (via APIM) - property listings, rental rates
              {
...
output "agent_location_scout_a2a_url" {
  description = "The public URL of the Location Scout A2A Agent"
  value       = "https://ca-agent-location-scout-a2a.${azapi_resource.container_app_environment.output.properties.defaultDomain}"
}

output "agent_location_scout_a2a_agent_card" {
  description = "The A2A Agent Card URL for discovery"
  value       = "https://ca-agent-location-scout-a2a.${azapi_resource.container_app_environment.output.properties.defaultDomain}/.well-known/agent-card.json"
}
